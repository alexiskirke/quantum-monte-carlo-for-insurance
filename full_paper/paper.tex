...
The pipeline begins by acquiring empirical loss data to parameterise the catastrophe distribution. Two data sources are supported. For Experiments~1--3, synthetic data is drawn from a heavy-tailed Pareto distribution with shape parameter $\alpha = 1.5$ and scale parameter \$50,000, values chosen to approximate the empirical severity profile of US hurricane property damage \cite{Grossi2005}; 20,000 records are generated. For Experiment~4, the pipeline dynamically discovers and downloads the latest NOAA Storm Events detail files \cite{NOAA2024} spanning 2020--2024, extracting all records with property damage $\geq$~\$1,000. To ensure reproducibility, the pipeline writes a pinned manifest (\texttt{data/cache/noaa\_manifest.json}) recording the exact filenames, base URL, and retrieval date; the files used in this paper are listed in Table~\ref{tab:noaa_files}. This yields 58,028 real loss records---a substantially more heavy-tailed distribution ($\hat{\sigma} = 2.02$, $\hat{\mu} = 9.04$, median $\approx$~\$8,465) than the synthetic Pareto ($\hat{\sigma} = 0.67$, $\hat{\mu} = 11.49$, median $\approx$~\$97,203).
...
With $k$ Grover iterations, the measurement probability is amplified to $\hat{p}_{\text{meas}} = \sin^2((2k+1)\theta)$ where $\theta = \arcsin(\sqrt{P(\ket{1})})$. The de-amplification correction
\begin{equation}
...
CT and IS exploit the closed-form lognormal CDF/PDF; they cannot be directly applied to non-parametric oracles without an additional density-estimation step (see Experiment~7 discussion). RMSE is computed over 50 repetitions against the analytic ground truth (item~1), which includes discretisation error for the bin-based methods.
...
\begin{table}[t]
\centering
\caption{Experiment~4B: error decomposition on real NOAA data ($n_{\text{reps}}=30$, $\approx$8,192 total queries). $C_{\text{cont}}$ samples the continuous lognormal (RMSE vs analytic); $C_{\text{bins}}$ and Q sample from the same 8-bin discretisation (RMSE vs exact-on-bins).}
\label{tab:exp4b}
...
However, this analysis assumes fault-tolerant execution of depth-200,000 circuits, efficient loading of the catastrophe model into a quantum oracle, and $n \gg 7$ qubits for adequate bin resolution. Each of these assumptions is far from near-term reality. The break-even is therefore a long-term prospect: QAE for catastrophe pricing becomes practically relevant when fault-tolerant hardware can execute circuits with $O(10^5)$ depth at microsecond gate times, and efficient state-preparation methods (qGAN, QROM) can encode high-dimensional loss distributions.
...
This paper has provided the first rigorous empirical evaluation of quantum amplitude estimation for catastrophe insurance tail-risk pricing, with full error decomposition and comparison against both naive and variance-reduced classical baselines. Seven experiments on synthetic and real NOAA Storm Events data yield three core findings: (i)~the oracle-model advantage is real and consistent (2--4$\times$ lower estimation RMSE across all qubit counts and data sources); (ii)~variance-reduced classical methods (CT, IS) dominate when analytical access to the loss distribution is available, but offer no direct advantage on empirical PMFs without extra modelling; and (iii)~discretisation error, not estimation error, is the current bottleneck at 3--7 qubits.
...
\bibitem{Kirke2025}
Kirke, A. (2025).
Applying foundational quantum computing algorithms to symbolic music generation: {HHL} and quantum {Monte Carlo}/amplitude-estimation.
\emph{arXiv preprint}.
