...
The LaTeX source is in `full_paper/paper.tex`. To build the PDF:
...
Plot images (`*.png`) and the compiled PDF are not tracked in version control; they are regenerated by the pipeline and `pdflatex` respectively.
